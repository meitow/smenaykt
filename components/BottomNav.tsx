...
export function BottomNav() {
  ...
  const tabCount = APP_NAV_TABS.length;
  ...
  return (
    ...
    >
      <div className="relative app-shell px-1 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2">
        {activeIndex >= 0 && (
          <div
            className="absolute top-2 h-12 rounded-2xl bg-brand-light/50 transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]"
            style={{
              width: `calc(${tabWidthPct}% - 0.35rem)`,
              left: `calc(${activeIndex * tabWidthPct}% + 0.2rem)`,
            }}
            aria-hidden
          />
        )}
          ...
          {APP_NAV_TABS.map((tab) => {
            ...
            return (
              <Link
                key={tab.href}
                href={tab.href}
                className={`relative z-10 flex min-w-0 flex-1 flex-col items-center gap-1 px-1 py-1.5 transition duration-200 ${
